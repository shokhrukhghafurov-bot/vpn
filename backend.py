...
def _subscription_truthy(value: Any) -> bool:
    if isinstance(value, bool):
        return value
    if isinstance(value, (int, float)):
        return bool(value)
...
def _subscription_payload_is_mix_profile(payload: Dict[str, Any]) -> bool:
...
def _subscription_mix_publish_enabled(payload: Dict[str, Any]) -> bool:
    ...
    if not _subscription_payload_is_mix_profile(data):
        return False
    if "publish_as_subscription" in data:
...
def _subscription_mix_replace_enabled(payload: Dict[str, Any]) -> bool:
    """Return True only for the old v13 behavior where MIX replaces the whole subscription.

    Default v14 behavior is append-safe: an active MIX row in admin must not remove
    or replace old vless:// locations in the normal /sub/<token> response.
    """
    data = dict(payload or {})
    mode = str(data.get("publish_mode") or data.get("publishMode") or data.get("mix_publish_mode") or "append").strip().lower()
    return mode in {"replace", "json", "override", "only"}


def _subscription_explicit_mix_request(request: Request) -> bool:
    """Manual/test switch for returning only the MIX Xray JSON profile.

    Normal bot/app flow still uses the same old /sub/<token> URL. When an active
    MIX row exists, /sub/<token> publishes a JSON-array subscription with the
    regular public locations plus the generated MIX profile appended. Query params
    like ?mix=1 are only for admin/debug preview of the single MIX profile.
    """
    try:
        params = request.query_params
    except Exception:
        return False
    for key in ("mix", "xray_json", "xray", "json_profile"):
        raw = params.get(key)
        if raw is not None and _subscription_truthy(raw):
            return True
    fmt = str(params.get("format") or params.get("profile_format") or "").strip().lower()
    return fmt in {"mix", "xray", "xray-json", "xray_json", "json"}
...
def _subscription_location_row_by_code(code: str) -> Optional[Dict[str, Any]]:
    wanted = str(code or "").strip()
    if not wanted:
        return None
    # MIX source lookup intentionally searches inactive rows too.
    # If admin disables ru-lte-01/ru-lte-1 as public locations, the active
    # mix-tg-youtube profile must still reuse their saved payloads and
    # per-device UUID credentials as internal source outbounds.
    for row in list_locations(active_only=False):
        row_code = str(row.get("code") or "").strip()
        if row_code == wanted:
            return dict(row)
...
def _active_mix_subscription_row() -> Optional[Tuple[Dict[str, Any], Dict[str, Any]]]:
    ...
    if not candidates:
        return None
    candidates.sort(key=lambda pair: (0 if bool(pair[0].get("is_recommended")) else 1, int(pair[0].get("sort_order") or 9999), str(pair[0].get("code") or "")))
    return candidates[0]


def _xray_stream_settings_from_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    normalized = _normalize_vpn_payload_keys(payload or {})
    transport = _payload_transport(normalized)
    security = _payload_security(normalized)
    server_name = str(normalized.get("server_name") or normalized.get("sni") or normalized.get("host") or "").strip()
    stream_settings: Dict[str, Any] = {"network": "ws" if transport == "websocket" else transport}
    if security == "reality":
        stream_settings["security"] = "reality"
        reality_settings: Dict[str, Any] = {
            ...
            "spiderX": _xray_reality_spider_x(normalized, transport) or "/",
        }
        alpn = [str(item or "").strip() for item in (normalized.get("alpn") or []) if str(item or "").strip()]
        if alpn:
        ...
        tls_settings: Dict[str, Any] = {
            "serverName": server_name,
            "allowInsecure": bool(normalized.get("allow_insecure") or normalized.get("allowInsecure") or False),
        }
        alpn = [str(item or "").strip() for item in (normalized.get("alpn") or []) if str(item or "").strip()]
        if alpn:
            tls_settings["alpn"] = alpn
    ...
    if transport == "grpc":
        stream_settings["grpcSettings"] = {
            "serviceName": str(normalized.get("service_name") or normalized.get("serviceName") or "").strip(),
            "multiMode": False,
        }
    elif transport in {"ws", "websocket"}:
        ws_settings: Dict[str, Any] = {"path": str(normalized.get("path") or "/").strip() or "/"}
        host = str(normalized.get("host") or server_name or "").strip()
        if host:
            ws_settings["headers"] = {"Host": host}
        stream_settings["wsSettings"] = ws_settings
    elif transport == "xhttp":
        ...
        if host:
            xhttp_settings["host"] = host
        extra = normalized.get("xhttp_settings")
        if isinstance(extra, dict):
            for key, value in extra.items():
                if key not in xhttp_settings and value not in (None, "", [], {}):
                    xhttp_settings[key] = value
...
def _xray_vless_outbound_from_payload(payload: Dict[str, Any], *, tag: str) -> Dict[str, Any]:
    normalized = _normalize_vpn_payload_keys(payload or {})
    if not _config_is_complete(normalized):
        raise ValueError(f"MIX source payload is incomplete for outbound {tag}")
    user: Dict[str, Any] = {
        "id": str(normalized.get("uuid") or normalized.get("id") or "").strip(),
        "encryption": str(normalized.get("encryption") or "none").strip() or "none",
    }
    flow = str(normalized.get("flow") or "").strip()
    if flow:
        user["flow"] = flow
    packet_encoding = str(normalized.get("packet_encoding") or normalized.get("packetEncoding") or "").strip()
    if packet_encoding:
        user["packetEncoding"] = packet_encoding
    return {
        "tag": tag,
        "protocol": str(normalized.get("protocol") or "vless").strip() or "vless",
        "settings": {
            "vnext": [
                {
                    "address": str(normalized.get("server") or "").strip(),
                    "port": int(normalized.get("port") or 443),
                    "users": [user],
                }
            ]
        },
        "streamSettings": _xray_stream_settings_from_payload(normalized),
    }


def _xray_direct_rule_for_server(payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    server = str((payload or {}).get("server") or "").strip()
    if not server:
        return None
    try:
        ipaddress.ip_address(server)
        return {"type": "field", "ip": [server], "outboundTag": "direct"}
    except ValueError:
        return {"type": "field", "domain": [f"domain:{server}"], "outboundTag": "direct"}


def _mix_telegram_domain_rules(payload: Dict[str, Any]) -> List[str]:
    custom = payload.get("telegram_domains") or payload.get("telegramDomains")
    if isinstance(custom, list):
        cleaned = [str(item or "").strip() for item in custom if str(item or "").strip()]
        if cleaned:
            return cleaned
    return [
        "geosite:telegram",
...
def _mix_default_proxy_domain_rules(payload: Dict[str, Any]) -> List[str]:
    custom = payload.get("default_proxy_domains") or payload.get("defaultProxyDomains")
    if isinstance(custom, list):
        cleaned = [str(item or "").strip() for item in custom if str(item or "").strip()]
        if cleaned:
            return cleaned
...
def _mix_xray_routing_rules(mix_payload: Dict[str, Any], telegram_payload: Dict[str, Any], default_payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    rules: List[Dict[str, Any]] = []
    for candidate in (telegram_payload, default_payload):
        direct_rule = _xray_direct_rule_for_server(candidate)
        if direct_rule and direct_rule not in rules:
            rules.append(direct_rule)
    rules.extend([
        {"type": "field", "domain": _mix_telegram_domain_rules(mix_payload), "outboundTag": "telegram"},
        {"type": "field", "domain": _mix_default_proxy_domain_rules(mix_payload), "outboundTag": "proxy"},
        {"type": "field", "ip": _probe_private_ip_cidrs(), "outboundTag": "direct"},
        {"type": "field", "domain": ["domain:localhost"], "outboundTag": "direct"},
        {"type": "field", "protocol": ["bittorrent"], "outboundTag": "block"},
    ])
    if _subscription_truthy(mix_payload.get("direct_ru", True)):
        insert_at = len(rules)
        if _ru_lte_geoip_direct_enabled():
            rules.insert(insert_at, {"type": "field", "ip": ["geoip:ru"], "outboundTag": "direct"})
            insert_at += 1
        if _ru_lte_geosite_direct_enabled():
            rules.insert(insert_at, {"type": "field", "domain": ["geosite:ru"], "outboundTag": "direct"})
            insert_at += 1
        rules.insert(insert_at, {"type": "field", "domain": _ru_split_domain_patterns(mix_payload), "outboundTag": "direct"})
    return rules


def _build_active_mix_xray_profile(
    *,
    user_id: Optional[int],
    device_id: Optional[int],
    legacy_static_uuid: bool,
    require_replace_mode: bool = False,
) -> Optional[Tuple[Dict[str, Any], List[Dict[str, Any]]]]:
    active = _active_mix_subscription_row()
    if not active:
        return None
    mix_row, mix_payload = active
    if require_replace_mode and not _subscription_mix_replace_enabled(mix_payload):
        return None
    # Do NOT publish a saved raw_xray_config from the MIX admin row as-is.
    # MIX must be built at subscription time from its source locations so the
    # outbounds receive this device's current per-device UUIDs. If an admin pastes
    # a raw MIX JSON that contains template/static UUIDs, using it directly would
    # break revocation: deleting the device would not reliably stop the copied
    # profile. The admin row may still keep raw_xray_config for notes/preview, but
    # runtime subscription output always resolves telegram/default source payloads.

    telegram_code, default_code = _subscription_mix_codes(mix_payload)
    telegram_row = _subscription_location_row_by_code(telegram_code)
    default_row = _subscription_location_row_by_code(default_code)
    if not telegram_row or not default_row:
        logger.warning(
            "[sub][mix] source location missing mix=%s telegram=%s found=%s default=%s found=%s",
            mix_row.get("code"), telegram_code, bool(telegram_row), default_code, bool(default_row),
        )
        return None

    def source_payload(row: Dict[str, Any]) -> Dict[str, Any]:
        if legacy_static_uuid:
            return _compose_vpn_payload_for_location(dict(row), requested_location_code=str(row.get("code") or "").strip() or None)
        if user_id is not None:
            return build_user_vpn_payload_for_location(
                int(user_id),
                dict(row),
                requested_location_code=str(row.get("code") or "").strip() or None,
                device_id=device_id,
            )
        return _compose_vpn_payload_for_location(dict(row), requested_location_code=str(row.get("code") or "").strip() or None)

    telegram_payload = source_payload(telegram_row)
    default_payload = source_payload(default_row)
    if not (telegram_payload and _config_is_complete(telegram_payload) and default_payload and _config_is_complete(default_payload)):
        logger.warning("[sub][mix] source payload incomplete mix=%s telegram=%s default=%s", mix_row.get("code"), telegram_code, default_code)
        return None

    dns_servers = mix_payload.get("dns_servers") or mix_payload.get("dnsServers") or default_payload.get("dns_servers") or default_payload.get("dnsServers") or ["1.1.1.1", "8.8.8.8"]
    dns_servers = [str(item or "").strip() for item in dns_servers if str(item or "").strip()] or ["1.1.1.1", "8.8.8.8"]
    remarks = str(mix_payload.get("remark") or mix_payload.get("display_name") or mix_row.get("name_en") or mix_row.get("name_ru") or mix_row.get("code") or "Russia MIX TG+YT").strip()
    config: Dict[str, Any] = {
        "log": {"loglevel": str(mix_payload.get("loglevel") or "warning").strip() or "warning"},
        "dns": {"servers": dns_servers, "queryStrategy": str(mix_payload.get("dns_query_strategy") or "UseIP").strip() or "UseIP"},
        "inbounds": [
            {
                "listen": "127.0.0.1",
                "port": int(mix_payload.get("socks_port") or 10808),
                "protocol": "socks",
            ...
            {
                "listen": "127.0.0.1",
                "port": int(mix_payload.get("http_port") or 10809),
                "protocol": "http",
        ...
        "outbounds": [
            _xray_vless_outbound_from_payload(telegram_payload, tag="telegram"),
            _xray_vless_outbound_from_payload(default_payload, tag="proxy"),
        ...
        "routing": {
            ...
            "domainStrategy": "IPIfNonMatch",
            "rules": _mix_xray_routing_rules(mix_payload, telegram_payload, default_payload),
        },
        "remarks": remarks,
        "meta": {
            "profile_type": "mix",
            "mix_profile": True,
            "mix_location_code": str(mix_row.get("code") or "").strip(),
            "telegram_location_code": telegram_code,
            "default_location_code": default_code,
            "telegram_route": "telegram",
            "default_route": "proxy",
            "direct_ru": _subscription_truthy(mix_payload.get("direct_ru", True)),
        },
    }
    return config, [mix_row, telegram_row, default_row]


def _single_xray_routing_rules(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    rules: List[Dict[str, Any]] = []
    direct_server_rule = _xray_direct_rule_for_server(payload)
    if direct_server_rule:
        rules.append(direct_server_rule)
    rules.extend([
    ...
    if _payload_direct_ru_enabled(payload):
        ...
        rules.append({"type": "field", "domain": ["geosite:ru"], "outboundTag": "direct"})
        rules.append({"type": "field", "domain": _ru_split_domain_patterns(payload), "outboundTag": "direct"})
    return rules


def _build_single_location_xray_profile(payload: Dict[str, Any], *, remark: str) -> Dict[str, Any]:
    normalized = _normalize_vpn_payload_keys(dict(payload or {}))
    if not _config_is_complete(normalized):
        raise ValueError("single location payload is incomplete")
    dns_servers = normalized.get("dns_servers") or normalized.get("dnsServers") or ["1.1.1.1", "8.8.8.8"]
    dns_servers = [str(item or "").strip() for item in dns_servers if str(item or "").strip()] or ["1.1.1.1", "8.8.8.8"]
    title = str(remark or normalized.get("remark") or normalized.get("display_name") or normalized.get("location_code") or "INET VPN").strip() or "INET VPN"
    config: Dict[str, Any] = {
        "log": {"loglevel": str(normalized.get("loglevel") or "warning").strip() or "warning"},
        "dns": {"servers": dns_servers, "queryStrategy": str(normalized.get("dns_query_strategy") or "UseIP").strip() or "UseIP"},
        "inbounds": [
            {
                "listen": "127.0.0.1",
                "port": int(normalized.get("socks_port") or 10808),
                "protocol": "socks",
                "settings": {"auth": "noauth", "udp": True},
                "sniffing": {"enabled": True, "destOverride": ["http", "tls", "quic"], "routeOnly": False},
                "tag": "socks",
            ...
            {
                "listen": "127.0.0.1",
                "port": int(normalized.get("http_port") or 10809),
                "protocol": "http",
                "settings": {"allowTransparent": False},
                "sniffing": {"enabled": True, "destOverride": ["http", "tls", "quic"], "routeOnly": False},
                "tag": "http",
        ...
        "outbounds": [
            _xray_vless_outbound_from_payload(normalized, tag="proxy"),
            {"tag": "direct", "protocol": "freedom", "settings": {"domainStrategy": "AsIs"}},
            {"tag": "block", "protocol": "blackhole"},
        ],
        "routing": {
            "domainMatcher": "hybrid",
            "domainStrategy": "IPIfNonMatch",
            "rules": _single_xray_routing_rules(normalized),
        },
        "remarks": title,
        "meta": {
            "profile_type": "single",
            "location_code": str(normalized.get("location_code") or normalized.get("locationCode") or normalized.get("resolved_location_code") or "").strip(),
            "direct_ru": _payload_direct_ru_enabled(normalized),
            "credential_mode": str(normalized.get("credential_mode") or "").strip(),
            "credential_location_code": str(normalized.get("credential_location_code") or "").strip(),
        },
    }
    return config


def _subscription_json_array_response(
    request: Request,
    token: str,
    *,
    ...
    for row in rows or []:
        ...
        if isinstance(raw_updated, datetime):
            ...
            try:
                ...
                parsed = None
        if parsed is not None and (last_modified_dt is None or parsed > last_modified_dt):
    ...
    headers = {
        "Content-Disposition": 'inline; filename="inet-xray-subscription.json"',
        ...
        "x-subscription-mode": "xray-json-array-with-mix",
    }
    response = Response(status_code=200, headers=headers) if head_only else Response(content=content, media_type="application/json; charset=utf-8", headers=headers)
    try:
        response.set_cookie(
            key="inet_sub_cid",
...
def _subscription_json_array_profiles(
    rows: List[Dict[str, Any]],
    *,
    mix_profile: Tuple[Dict[str, Any], List[Dict[str, Any]]],
    user_id: Optional[int],
    device_id: Optional[int],
    legacy_static_uuid: bool,
    ...
    for row in rows or []:
        payload_for_subscription, fallback_name = _subscription_payload_and_fallback_name(
            ...
            user_id=user_id,
            device_id=device_id,
            legacy_static_uuid=bool(legacy_static_uuid),
        )
        if not payload_for_subscription:
            continue
        if _subscription_payload_is_mix_profile(payload_for_subscription):
            continue
        if not _subscription_transport_allowed(payload_for_subscription, client_hint):
            continue
        try:
            config = _build_single_location_xray_profile(payload_for_subscription, remark=fallback_name)
        except Exception:
            continue
        code = str((config.get("meta") or {}).get("location_code") or row.get("code") or "").strip().lower()
    ...
    mix_config, mix_rows = mix_profile
    mix_code = str((mix_config.get("meta") or {}).get("mix_location_code") or "mix-tg-youtube").strip().lower()
    if mix_code and mix_code not in seen_codes:
        configs.append(mix_config)
        seen_codes.add(mix_code)
    used_rows.extend([dict(row) for row in (mix_rows or []) if isinstance(row, dict)])
    return configs, used_rows


def _subscription_json_profile_response(
    request: Request,
    token: str,
    *,
    config: Dict[str, Any],
    ...
    response_etag = hashlib.sha256(content.encode("utf-8")).hexdigest()
    headers = {
        "Content-Disposition": 'inline; filename="inet-mix-xray.json"',
        "Profile-Title": quote(f"base64:{base64.b64encode(profile_title.encode('utf-8')).decode('ascii')}", safe=':='),
        "Subscription-Userinfo": subscription_userinfo,
        "Cache-Control": "private, no-store, no-cache, must-revalidate, proxy-revalidate, max-age=0, s-maxage=0",
        "CDN-Cache-Control": "no-store, no-cache, max-age=0",
        ...
        "x-subscription-version": content_version,
        "x-subscription-generated-at": datetime.now(timezone.utc).isoformat(),
        "x-subscription-mode": "mix-xray-json",
    }
    response = Response(status_code=200, headers=headers) if head_only else Response(content=content, media_type="application/json; charset=utf-8", headers=headers)
    try:
        response.set_cookie(
...
def _subscription_response(request: Request, token: str, *, head_only: bool = False) -> Response:
    ...
    explicit_mix_request = _subscription_explicit_mix_request(request)
    mix_profile = _build_active_mix_xray_profile(
        user_id=subscription_user_id,
        device_id=subscription_device_id,
        legacy_static_uuid=bool(legacy_user_token_grace),
        require_replace_mode=False,
    )
    rows = _subscription_location_rows(subscription_user_id, client_hint=client_hint)

    if mix_profile and explicit_mix_request:
        # Admin/debug preview only. The normal bot link does not need a second URL.
        mix_config, mix_rows = mix_profile
        if subscription_device_id:
            _trigger_xray_sync("subscription_generated", user_id=subscription_user_id, device_id=subscription_device_id)
        if should_track_real_connected:
            _record_real_connected_locations(request, token, access, mix_rows, fingerprint=session_fingerprint)
        return _subscription_json_profile_response(
            request,
            token,
            config=mix_config,
            rows=mix_rows,
    ...
    if mix_profile:
        # Active MIX is published through the same old /sub/<token> link as one
        # JSON-array subscription: all normal locations remain as separate
        # profiles, and mix-tg-youtube is appended as another profile.
        json_configs, json_rows = _subscription_json_array_profiles(
            rows,
        ...
        if json_configs:
            if subscription_device_id:
                _trigger_xray_sync("subscription_generated", user_id=subscription_user_id, device_id=subscription_device_id)
            if should_track_real_connected:
                _record_real_connected_locations(request, token, access, json_rows, fingerprint=session_fingerprint)
            return _subscription_json_array_response(
                request,
                token,
    ...
    for row in rows:
        ...
        if not payload_for_subscription:
            continue
        if _subscription_payload_is_mix_profile(payload_for_subscription):
            # MIX rows are real admin locations, but they are not single-server
            # VLESS endpoints. If active MIX exists, the response returned above
            # is a JSON-array subscription. Without active MIX, skip the row so
            # normal vless:// publishing stays safe.
            continue
        try:
